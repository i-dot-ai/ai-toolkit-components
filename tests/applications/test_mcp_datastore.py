...
class TestMcpDatastore:
    ...
    def test_service_health(self, application_endpoint):
        ...
        assert application_endpoint.verify_health("vector_db"), "vector_db is unhealthy"
        assert application_endpoint.verify_health("mcp_server"), "mcp_server is unhealthy"
    ...
    def test_mcp_server_health(self, application_endpoint):
        ...
        import requests
        response = requests.get(f"{application_endpoint.url}/health", timeout=10)
    ...
    def test_custom_tool_is_discovered_and_callable(self, application_endpoint, custom_tool_code):
        ...
        assert application_endpoint.verify_health("mcp_server"), "mcp_server is unhealthy after restart"

        # Check container logs for tool registration
        logs = application_endpoint.logs("mcp_server")
